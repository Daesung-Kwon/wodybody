...
| **3 — Phase C** | Playwright E2E / 분석 이벤트 / Sprint 3 종결 | ✅ **완료 (코드)** — Playwright E2E 핵심 경로 1스펙(11단계, `page.route` 모킹·`.env.test` 이중 격리·운영 호출 0건) + CI 배선 / 분석 이벤트 6종(`lib/analytics.ts` `track()` 래퍼·PII 필터·Plausible) / vitest 79→86 + e2e 1/1. ⚠️ `VITE_ANALYTICS_DOMAIN` 환경변수 등록 필요. **Sprint 3 공식 종결** — 후속은 [`burnfat/docs/ROADMAP_NEXT_2026-Q3.md`](burnfat/docs/ROADMAP_NEXT_2026-Q3.md). 상세: [`burnfat/docs/SPRINT3_FINAL_HANDOFF.md`](burnfat/docs/SPRINT3_FINAL_HANDOFF.md) |
