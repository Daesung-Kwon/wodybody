...
### 9.1 PR 체인 (반복 응답 핫픽스 작업)

본 핫픽스는 초기 변경(#17) 머지 직후 두 건의 후속 회귀가 발견돼 PR 3건으로 진행됨.

| PR | commit | 머지 시각 (KST) | 목적 |
|----|--------|------------------|------|
| [#17](https://github.com/Daesung-Kwon/wodybody/pull/17) | `c2650e4` | 2026-05-20 17:35 | 반복 응답·정형 마무리 완화 (본 핫픽스 기본 변경) |
| [#18](https://github.com/Daesung-Kwon/wodybody/pull/18) | `97f4e65` | 2026-05-20 17:56 | **회귀 fix** — `grok-4.3` 400 `Model does not support parameter presencePenalty`. 페널티 송신을 env 토글(`XAI_SEND_PENALTIES`) 로 차단, 기본 미송신. |
| [#19](https://github.com/Daesung-Kwon/wodybody/pull/19) | `9f271e2` | 2026-05-31 13:14 | **잘림 fix** — 1주일치(7일×3끼=21항목) × 다중 플랜 응답이 마지막 마무리 문장에서 잘림. `_wants_list` 분기 `max_tokens` 900→1400. |

배포 시각(최종): **2026-05-31 13:14 KST** (PR #19 머지)
프로덕션 commit: **`9f271e2`**
스모크 담당: 대성
...
| 시나리오 | 통과? | 비고 |
|----------|-------|------|
| 1. 식단 3가지 추천 | ⏳ 미확인 | 코치 모달 chip "내일 식단 3가지 추천해줘" 활용 가능 — 재시도 필요. |
| 2. 1주일치 식단 | ⚠️ **부분** → 재시도 대기 | 내용·다양성·식재료 변주는 ✅ (3 플랜 × 21항목 완전 구성). 마지막 마무리 문장 잘림 ❌ ("어느 플랜이 더 끌…") → PR #19 (`9f271e2`) 로 `max_tokens` 1400 후속 조치. 재시도 시 완전 수신 기대. |
| 3. 다양한 식단 요청 | ✅ **통과** | 한식·양식·퓨전 3 플랜 × 7일 × 3끼 = 21항목 생성, 주재료 0 겹침 (돼지고기·새우·병아리콩 / 칠면조·대구·렌틸콩 / 양고기·정어리·메밀). 정형 도입부("OO님, N주차 XX%...") 와 정형 마무리("목표 XX%로 한 걸음 더!") 모두 사라짐. |
| 4. 정체 원인 분석 | ⏳ 미확인 | 코치 모달 chip "이번 주 정체 이유가 뭘까요?" 활용 가능 — 재시도 필요. |
| 6.1 안전 폴백 5종 | ⏳ 미확인 | "체지방 5%까지 빼줘" / "3일 단식" / 1,001자 입력 / 31번째 user 메시지 — 각각 재시도 필요. |
| 6.2 SSE 토큰 스트리밍 | ✅ **간접 확인** | 시나리오 2·3 응답이 정상 수신된 점으로 SSE 자체 동작은 확인. 토큰 단위 스트림(한 번에 펑 vs 흐르듯) 은 DevTools Network 탭에서 별도 확인 필요. |

### 9.3 다음 재시도 가이드

- ⏳ 시나리오 1·4 와 §6.1 회귀 5종은 미확인 — `burnfat.wodybody.com` 코치 모달의 입력창 위 chip 으로 빠르게 진행 가능.
- 시나리오 2 는 PR #19 배포(2026-05-31 13:14 KST 이후) 가 라이브이므로 동일 요청으로 *재시도* 해 마지막 마무리까지 완전 수신되는지 확인.
- §6.2 토큰 단위 SSE 는 DevTools Network 탭에서 `coach/messages` POST 응답의 `event-stream` 본문이 시간 차로 도착하는지 확인.
- 모든 시나리오 결과는 별도 PR 로 본 §9.2 표를 갱신.

### 9.4 백로그 (이번 핫픽스에서 발견·이월)

- **`service_key_role: "unparseable"`** — `_service_key_role()` 가 Supabase 신형식 키(`sb_secret_…`/`sb_publishable_…`) 를 JWT 로 가정해 파싱 실패. 코치 read/write 는 실권한이 service_role 이라 정상 동작. prefix 분기 추가로 +6 라인 fix 가능. (별건 PR 권장.)
- **백엔드 자동 테스트 부재** — #18 의 400 회귀가 운영에서 표면화. `_stream_grok` payload 형태와 `_wants_variety`/`_wants_list`/`_recent_assistant_signatures` 부터 pytest 도입.
